#!/usr/bin/env python3
"""
Script tạo admin user với role đúng
"""
...
def delete_all_admins():
    """Xóa tất cả admin"""
    print("🗑️  Đang xóa tất cả admin...")
    
    db = SessionLocal()
    try:
        db.execute(text("DELETE FROM users WHERE email LIKE '%admin%'"))
        db.commit()
        print("✅ Tất cả admin đã được xóa")
        return True
...
def create_admin_with_correct_role():
    """Tạo admin với role đúng"""
    print("👤 Đang tạo admin với role đúng...")
    ...
    try:
        ...
        # Tạo admin với role 'ADMIN' (không phải 'admin')
        db.execute(text("""
...
def main():
    """Hàm chính"""
    print("🚀 Tạo admin với role đúng và test authentication")
    print("=" * 60)
    
    # Bước 1: Xóa tất cả admin
    if not delete_all_admins():
        return False
    
    # Bước 2: Tạo admin mới với role đúng
    if not create_admin_with_correct_role():
        return False
